...
async def get_next_candidate(
    session_id: UUID, service: EmployerService = Depends(dependencies.get_service)
):
...
@router.get(
    "/contact-requests/{request_id}/details",
    response_model=schemas.ContactDetailsRequest,
)
async def get_contact_request_details(
    request_id: UUID, service: EmployerService = Depends(dependencies.get_service)
):
