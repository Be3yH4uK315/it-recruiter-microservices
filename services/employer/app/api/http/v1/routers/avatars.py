...
from app.api.http.v1.dependencies import (
    get_delete_employer_avatar_handler,
    get_employer_avatar_upload_url_handler,
    get_get_employer_handler,
    get_replace_employer_avatar_handler,
)
from app.application.employers.commands.delete_employer_avatar import (
    DeleteEmployerAvatarCommand,
    DeleteEmployerAvatarHandler,
)
from app.application.employers.commands.replace_employer_avatar import (
    ReplaceEmployerAvatarCommand,
...
from app.application.employers.queries.get_employer_avatar_upload_url import (
    ...
    GetEmployerAvatarUploadUrlQuery,
)
...
router = APIRouter(
    prefix="/employers/{employer_id}/avatar",
    tags=["employer-avatar"],
)
...
async def get_employer_avatar_upload_url(
    ...
    handler: GetEmployerAvatarUploadUrlHandler = Depends(get_employer_avatar_upload_url_handler),
) -> UploadUrlResponse:
...
async def replace_employer_avatar(
    employer_id: UUID,
    payload: ReplaceFileRequest,
    subject_telegram_id: int = Depends(require_employer_subject),
    employer_handler: GetEmployerHandler = Depends(get_get_employer_handler),
    handler: ReplaceEmployerAvatarHandler = Depends(get_replace_employer_avatar_handler),
) -> Response:
    employer = await employer_handler(employer_id)
    if employer.telegram_id != subject_telegram_id:
        raise HTTPException(
...
async def delete_employer_avatar(
    employer_id: UUID,
    subject_telegram_id: int = Depends(require_employer_subject),
    employer_handler: GetEmployerHandler = Depends(get_get_employer_handler),
    handler: DeleteEmployerAvatarHandler = Depends(get_delete_employer_avatar_handler),
) -> Response:
    employer = await employer_handler(employer_id)
    if employer.telegram_id != subject_telegram_id:
        raise HTTPException(
    ...
    await handler(DeleteEmployerAvatarCommand(employer_id=employer_id))
    return Response(status_code=status.HTTP_204_NO_CONTENT)
