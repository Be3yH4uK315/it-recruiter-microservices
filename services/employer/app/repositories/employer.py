...
class EmployerRepository:
    ...
    async def get_by_telegram_id(self, telegram_id: int) -> Optional[models.Employer]:
        query = select(models.Employer).where(models.Employer.telegram_id == telegram_id)
        result = await self.session.execute(query)
        return result.scalars().first()
        
    async def get_by_id(self, employer_id: UUID) -> Optional[models.Employer]:
    ...
    async def update(self, employer_id: UUID, update_data: schemas.EmployerUpdate) -> Optional[models.Employer]:
    ...
    async def get_session(self, session_id: UUID) -> Optional[models.SearchSession]:
    ...
    async def get_contact_request(self, employer_id: UUID, candidate_id: UUID) -> Optional[models.ContactsRequest]:
    ...
    async def get_request_with_employer_tg(self, request_id: UUID):
        ...
        query = (
            ...
            .join(models.Employer, models.ContactsRequest.employer_id == models.Employer.id)
            .where(models.ContactsRequest.id == request_id)
        )
        result = await self.session.execute(query)
        return result.first()