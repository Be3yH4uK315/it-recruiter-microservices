...
class IndexerService:
    def __init__(self):
        ...
        self._shadow_index_name: str | None = None
    ...
    async def process_candidate_update(self, candidate_data: dict[str, Any]):
        ...
        await self.es_client.index(
            index=settings.CANDIDATE_INDEX_ALIAS, id=candidate_id, document=es_doc
        )

        if self._shadow_index_name:
            try:
                await self.es_client.index(
                    index=self._shadow_index_name, id=candidate_id, document=es_doc
                )
    ...
    async def delete_candidate(self, candidate_id: str):
        await self.es_client.delete(
            index=settings.CANDIDATE_INDEX_ALIAS, id=candidate_id, ignore=[404]
        )
        if self._shadow_index_name:
            await self.es_client.delete(
                index=self._shadow_index_name, id=candidate_id, ignore=[404]
            )
            ...
            async with httpx.AsyncClient(timeout=60.0) as client:
                while True:
                    ...
                    for cand in candidates:
                        ...
                        await self.es_client.index(
                            index=new_index_name, id=cand["id"], document=es_doc
                        )
            ...
            try:
                ...
                if is_exists:
                    ...
                    if alias_name in index_info:
                        logger.warning(f"Found physical index '{alias_name}'. \
                                Deleting it to make room for alias...")
    ...
    def _prepare_text_for_embedding(self, data: dict) -> str:
        parts = []
        if role := data.get("headline_role"):
