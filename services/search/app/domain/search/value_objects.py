...
class SearchFilters:
    ...
    def __post_init__(self) -> None:
        ...
        normalized_location = self._normalize_optional_text(self.location)
        object.__setattr__(self, "location", normalized_location)

        normalized_english = self._normalize_optional_text(self.english_level)
        object.__setattr__(
            self,
            "english_level",
            normalized_english.upper() if normalized_english is not None else None,
        )

        normalized_about_me = self._normalize_optional_text(self.about_me)
        object.__setattr__(self, "about_me", normalized_about_me)
