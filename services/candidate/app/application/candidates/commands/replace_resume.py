...
class ReplaceResumeCommand:
    candidate_id: UUID
    file_id: UUID


class ReplaceResumeHandler:
    def __init__(
        self,
    ...
    async def __call__(self, command: ReplaceResumeCommand) -> None:
        ...
        if file_metadata.category != "candidate_resume":
            raise InvalidCandidateFileError("file category is invalid for resume")
        ...
        async with self._uow_factory() as uow:
            ...
            old_file_id = candidate.replace_resume(file_id=command.file_id)
        ...
        if old_file_id is not None and old_file_id != command.file_id:
            await self._file_gateway.cleanup_file(
                file_id=old_file_id,
                reason="candidate_resume_replaced",
            )
