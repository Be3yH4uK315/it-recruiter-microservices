...
Telegram бот для IT рекрутинга. Позволяет кандидатам создавать профили и работодателям подбирать специалистов через FSM-based interface в Telegram.

## Описание

Двурольный Telegram бот с Finite State Machine архитектурой:

**Для кандидатов**: Регистрация и редактирование профилей через многошаговый FSM

- Базовая информация (имя, роль, локация)
- Иерархические данные (опыт, навыки, проекты, образование)
- Загрузка резюме и аватара
- Редактирование контактов

**Для работодателей**: Поиск и просмотр кандидатов

- Ввод фильтров поиска (роль, навыки, опыт, локация, зарплата)
- Просмотр кандидатов (с аватаром и полной информацией)
- Принятие решений (лайк/пас/контакт запрос)

## Технологический стек

- **Bot Framework**: aiogram 3.5.0
- **State Management**: Redis (FSM storage + token caching)
- **HTTP Client**: httpx (aio) с tenacity retry
- **Data Validation**: Pydantic v2
- **Testing**: pytest + pytest-asyncio + respx
- **Performance**: Locust для load testing
